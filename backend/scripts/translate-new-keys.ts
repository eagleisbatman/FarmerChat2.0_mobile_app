...
async function translateNewKeys() {
  ...
  // Get all unique keys that exist only in English
  const keysResult = await pool.query(`
    SELECT DISTINCT k.key, k.translation 
    FROM ui_translations k
    WHERE k.language_code = 'en'
    AND k.key NOT IN (
      SELECT DISTINCT key 
  ...
  // Show the keys to be translated
  logger.info('\nKeys to translate:');
  newKeys.forEach(row => {
    logger.info(`  - ${row.key}: "${row.translation}"`);
  });
  ...
  for (const lang of SUPPORTED_LANGUAGES.filter(l => l.code !== 'en')) {
    languageCount++;
    logger.info(`\n[${languageCount}/${SUPPORTED_LANGUAGES.length - 1}] Translating to ${lang.name} (${lang.native})...`);
...
Return ONLY this JSON format:
{
  ...
  ...
        ...
        try {
          ...
          logger.info(`  Batch ${Math.floor(i/batchSize) + 1}/${Math.ceil(newKeys.length/batchSize)} ✓`);
        } catch (error) {

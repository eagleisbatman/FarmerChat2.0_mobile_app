...
async function syncAllStringKeys() {
    logger_1.logger.info('Starting complete string key synchronization...');
    // First, add English translations for all keys
    const client = await database_1.pool.connect();
    try {
        await client.query('BEGIN');
        logger_1.logger.info('Adding all 267 string keys with English translations...');
        for (const [key, value] of Object.entries(ALL_STRING_KEYS)) {
            await client.query(`INSERT INTO ui_translations (language_code, key, translation)
         VALUES ($1, $2, $3)
         ON CONFLICT (language_code, key)
         DO UPDATE SET translation = $3, updated_at = NOW()`, ['en', key, value]);
        }
        await client.query('COMMIT');
        logger_1.logger.info('✓ Added all English translations');
    }
    catch (error) {
    ...
    const result = await database_1.pool.query('SELECT language_code, key FROM ui_translations WHERE language_code != $1', ['en']);
    ...
    for (const lang of SUPPORTED_LANGUAGES) {
        ...
        if (missingKeys.length === 0) {
            logger_1.logger.info(`${lang.name}: All keys already translated ✓`);
            continue;
        }
        logger_1.logger.info(`${lang.name}: Translating ${missingKeys.length} missing keys...`);
...
Return ONLY this JSON format:
{
  ...
  ...
               ...
               VALUES ($1, $2, $3)
               ON CONFLICT (language_code, key)
               DO UPDATE SET translation = $3, updated_at = NOW()`, [lang.code, key, cleanTranslation]);
                    }
                    await client.query('COMMIT');
                    logger_1.logger.info(`  Batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(missingKeys.length / batchSize)} ✓`);
                }
                catch (error) {
                    await client.query('ROLLBACK');
                    throw error;
                ...
                logger_1.logger.error(`Failed to translate batch for ${lang.name}:`, error);
            }
        }
    }
    // Final verification
    const finalResult = await database_1.pool.query(`
    SELECT 
  ...
  `);
    ...
    logger_1.logger.info(`Total unique keys: ${stats.total_keys} (expected: 267)`);
    logger_1.logger.info(`Total languages: ${stats.total_languages} (expected: 53)`);
    logger_1.logger.info(`Total translations: ${stats.total_translations} (expected: ${267 * 53} = 14,151)`);
    // Check which languages have all keys
    const completenessResult = await database_1.pool.query(`
    SELECT 
      language_code,
  ...
  `);
    ...
    for (const row of completenessResult.rows) {
        ...
        logger_1.logger.info(`${lang?.name || row.language_code}: ${status}`);
    }
}
// Main execution
if (require.main === module) {
    syncAllStringKeys()
        ...
        .catch(error => {
        logger_1.logger.error('Synchronization failed:', error);
        process.exit(1);
    });
}
